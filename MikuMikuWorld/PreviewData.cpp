...
namespace MikuMikuWorld::Engine
{
	...
	void DrawData::calculateDrawData(Score const& score)
	{
		...
		try
		{
			...
			for (int layer = 0; layer < score.layers.size(); ++layer)
			{
				...
				std::sort(hsList.begin(), hsList.end(),
				...
				{
					double stm = accumulateScaledDuration(tick, TICKS_PER_BEAT, score.tempoChanges,
					                                      score.hiSpeedChanges, layer);
...

			{
				...
				}
				{
					...
					for (const auto& [tick, skill] : skillList)
					{
						...
						hudSkillEffects.push_back(static_cast<uint8_t>(skill->effect));
			...
			// =========================================================================
			// 【変更】同じTickに存在するノーツの <中心X座標, レイヤー, ステージID>
			// のリストを保持する
			// =========================================================================
			std::map<int, std::vector<std::tuple<float, int, id_t>>> simBuilder;

			for (const auto& [id, note] : score.notes)
			{
				if (note.layer >= 0 && note.layer < score.layers.size() &&
				...
				drawingNotes.push_back(
				    DrawingNote{ note.ID, visual_tm, type, note.dummy, note.layer });

				// 同時押し線構築のためにリストに追加する
				float center = getNoteCenter(note);
				simBuilder[note.tick].push_back({ center, note.layer, note.stageID });
			}
			...
			for (const auto& [line_tick, notesAtTick] : simBuilder)
			{
				...
				if (notesAtTick.size() > 1)
				{
					...
					if (std::get<0>(*minmax.first) != std::get<0>(*minmax.second))
					{
						drawingLines.push_back(DrawingLine{
						    line_tick,                  // leftTick
						    std::get<0>(*minmax.first), // leftLane
						    std::get<1>(*minmax.first), // leftLayer
						    std::get<2>(*minmax.first), // leftStageID

						    line_tick,                   // rightTick
						    std::get<0>(*minmax.second), // rightLane
						    std::get<1>(*minmax.second), // rightLayer
						    std::get<2>(*minmax.second)  // rightStageID
						});
					}
				}
			}
	...
	void addHoldNote(DrawData& drawData, const HoldNote& holdNote, Score const& score)
	{
		...
		if (startNote.layer >= 0 && startNote.layer < score.layers.size() &&
		...
		DrawingHoldStep head = { startNote.tick,
			                     accumulateScaledDuration(startNote.tick, TICKS_PER_BEAT,
			                                              score.tempoChanges, score.hiSpeedChanges,
			                                              startNote.layer),
			                     Engine::laneToLeft(startNote.lane),
			                     Engine::laneToLeft(startNote.lane) + startNote.width,
			                     holdNote.start.ease };

		for (ptrdiff_t headIdx = -1, tailIdx = 0, stepSz = holdNote.steps.size(); headIdx < stepSz;
		     ++tailIdx)
		{
			if (tailIdx < stepSz && holdNote.steps[tailIdx].type == HoldStepType::Skip)
				continue;
			HoldStep tailStep = tailIdx == stepSz ? HoldStep{ holdNote.end, HoldStepType::Hidden }
			                                      : holdNote.steps[tailIdx];
			const Note& tailNote = score.notes.at(tailStep.ID);
			auto easeFunction = getEaseFunction(head.ease);
			DrawingHoldStep tail = {
				tailNote.tick,
				accumulateScaledDuration(tailNote.tick, TICKS_PER_BEAT, score.tempoChanges,
				                         score.hiSpeedChanges, startNote.layer),
				Engine::laneToLeft(tailNote.lane),
				Engine::laneToLeft(tailNote.lane) + tailNote.width, tailStep.ease
			};
			...
			drawData.drawingHoldSegments.push_back(DrawingHoldSegment{
			    holdNote.end, head.ease, holdNote.isGuide(), holdNote.guideColor, holdNote.dummy,
			    startNote.layer, tailIdx, head.time, tail.time, head.left, head.right, tail.left,
			...
			while ((headIdx + 1) < tailIdx)
			{
				...
				if (skipNote.tick > tail.tick)
					break;
				double tickTime =
				    accumulateScaledDuration(skipNote.tick, TICKS_PER_BEAT, score.tempoChanges,
				                             score.hiSpeedChanges, startNote.layer);
				...
				drawData.drawingHoldTicks.push_back(DrawingHoldTick{
				    skipStep.ID, skipLeft + (skipRight - skipLeft) / 2,
				    Range{ tickTime - noteDuration, tickTime }, holdNote.dummy, startNote.layer });
				++headIdx;
			}
			if (tailStep.type != HoldStepType::Hidden)
			{
				double tickTime =
				    accumulateScaledDuration(tailNote.tick, TICKS_PER_BEAT, score.tempoChanges,
				                             score.hiSpeedChanges, startNote.layer);
				drawData.drawingHoldTicks.push_back(
				    DrawingHoldTick{ tailNote.ID,
